...
class layer_base:
    ...
    @classmethod
    def toMPO(cls,layer:qtn.TensorNetwork,input_idx:str,output_idx:str,bond_idx:str,single_tensor_tag):
        raise NotImplementedError("called the pure virtual base class")
...
class brick_layer(layer_base):
    def __call__(self,input_idx, output_idx,Nlink, min_layer_number=0,dtype=jnp.float64,op_tag='L{}',layer_tag = 'L', left_right_index = False) -> Union[qtn.TensorNetwork,tuple[qtn.TensorNetwork,dict,dict]]:
        ...
        if Nlink > 1:
            ...
            out &= qtn.Tensor(jnp.copy(self.data),inds = (*left,*right),tags= (utag,layer_tag))
    ...
    def get_inds_with_i(inds:list[str],i:int):
        ...
        istr = str(i)
        for ind in inds:
            if ind[-len(istr):] == istr and not isdigit(ind[-len(istr)-1]):
                out.append(ind)
        return out
    ...
    def toMPO(cls,layer:qtn.TensorNetwork,input_idx:str,output_idx:str,bond_idx:str,single_tensor_tag):
        ...
        for i in range(1,len(layer.tensors)):
            ...
            tensors.append(V)
        tensors[-1] = tensors[-1].data
        return qtn.MatrixProductOperator(tensors,'ldur',lower_ind_id = input_idx,upper_ind_id=output_idx)

        
class V_layer(layer_base):
    def __call__(self,input_idx, output_idx,Nlink,dtype=jnp.float64,cpt_inds = False) -> qtn.TensorNetwork:
        out = qtn.TensorNetwork([])
        ...
        if Nlink > 1:
            ...
            for i in range(Nlink-2,0,-1):
                ...
                out &= qtn.Tensor(data =jnp.copy(self.data), inds = [Xinner.format(i),Rinner.format(i+1),Rinner.format(i),output_idx.format(i+1)],tags=['L{}'.format(LR),"MR{}".format(i),'MR'])
                left_inds['L{}'.format(LR)] = (Xinner.format(i),Rinner.format(i+1))
                right_inds['L{}'.format(LR)] = (Rinner.format(i),output_idx.format(i+1))
            i = 0
            LR+=1
            ...
            right_inds['L{}'.format(i)] = (Xinner.format(i),Linner.format(i+1))
            out &= qtn.Tensor(data =jnp.copy(self.data), inds = [Xinner.format(i),Rinner.format(i+1),output_idx.format(i),output_idx.format(i+1)],tags=['L{}'.format(LR),"MR{}".format(i),'MR'])
            left_inds['L{}'.format(LR)] = (Xinner.format(i),Rinner.format(i+1))
            right_inds['L{}'.format(LR)] = (output_idx.format(i),output_idx.format(i+1))
        else:
...
def Iterative_MPOGatesA(mpo:qtn.MatrixProductOperator,precision:float,maxlayers:int=40,layer_gen = staircaselayer):
    ...
    while error>precision and layercount < maxlayers:
        ...
        layer = layer_SVD_optimizer(layer,left_inds,accumulator,mpo,Layer_ind_id,10,1e-4) #Current behavior is anomalous. we're using the same function, so it's the layer&accumulator combo that is at fault.
        layercount += 1
        ...
        mpo_left_layer = layer_gen.toMPO(left_layer,acculow,next_acculow,left_llid,Lop_tag)#the indices are wrong
        mpo_right_layer = layer_gen.toMPO(right_layer,accuhigh,next_accuhigh,right_llid,Rop_tag)

...
def Check_and_correct_ps(p: NDArray, s: NDArray) -> tuple[NDArray, NDArray, bool]:
    ...
    if (
        ...
        new_p = np.zeros(lp * 2 - 1)
        new_p[:lp:2] = p[: n + 1]
        new_p[lp::2] = p[n + 1 :]
        new_s = np.zeros(lp * 2 - 1)
        new_s[:lp:2] = s[: n + 1]
        new_s[lp::2] = s[n + 1 :]
...
def get_rotations_matrices(function, domain, max_layers, precision=1e-4):
    ...
    U, doubled = cpt_rotations(p, s, precision)
    return U, doubled
...
class get_rotations:
    def __init__(self, function, domain, max_layer, precision=1e-14):
        self.U, self.doubled = get_rotations_matrices(
...
def qubitize(
    ...
    """
    U, pi_over_2_domain = cpt_rotations_matrices(
    ...
    for u in U[:-1]:
        ...
        prev_matrix = u
    ugate = UnitaryGate(np.conj(U[-1].T) @ prev_matrix)
    circ.append(ugate, [nqbits - 1])
    return circ

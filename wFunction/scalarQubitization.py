...
def opt_qubitizetrealangles(function, domain, precision, max_layers, Kernel=None):
    ga = get_angles(
        lambda x: 0.95 * function(x), domain, max_layers, precision
    )  # ,Kernel = Kernel)
    phi = [*ga]
    n_layer = len(phi) - 1
...
def cpt_complement(fastfourierseries):
    ...
    n = len(roots)
    assert (
        np.abs(n / m - 0.5) < 1e-15
    ), " The root finding step is unstable, perhaps the absolute value of your function comes too close to 1 "
...
def cpt_Laurent_mats(function, domain, max_layers, precision, Kernel=None):
    ...
    ), "The algorithm cannot reach target precision {}. Try the variationnal method instead.".format(
        precision
    )
    assert (
    ...
    for u in rots[1:]:
        ...
        if alternate_orders:
            T, T_r = (
                T_r,
                T,
            )  # it should require fewr swaps to alternate between the two order with linear connectivity
        ugate = UnitaryGate(u)
        circ.append(ugate, [nqbits - 1])
    return circ
